...
TOKEN = "MTQ2MDQyMjA2NDEwNzQyMTg1OQ.GgJZ0X.N2h0jRY2EDVkpv4a1nl166MM1G1HSE0mRTTbKE"
...
class HarmixBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            log("✅ Lavalink connected", "SUCCESS")
    ...
    async def on_ready(self):
        ...
        log(f"📡 Latency: {round(self.latency * 1000)}ms | Servers: {len(self.guilds)}", "READY")
        log("=" * 50, "READY")
...
# ============== VOICE CONNECTION - WORKAROUND FOR WAVELINK 3.x BUG ==============
async def connect_voice(interaction: discord.Interaction):
    """
    WORKAROUND for Wavelink 3.x voice connection bug.
    Issue: Bot joins channel but connection event never fires (30s timeout)
    Fix: Use manual connection with shorter timeout and retry
    """
    ...
    # WORKAROUND: Try connecting with manual timeout handling
    try:
        log(f"🔄 Connecting to '{voice_channel.name}'...")
        
        # Create player manually first
        player = wavelink.Player(client=bot, channel=voice_channel)
        
        # Connect with shorter timeout (10s instead of 30s)
        # This prevents the long hang and gives us faster feedback
        await asyncio.wait_for(
            voice_channel.connect(cls=wavelink.Player, timeout=10.0),
            timeout=15.0  # Hard cutoff at 15s
        )
        ...
        log(f"⏱️ Connection timed out", "ERROR")
        bot_stats["failed_connections"] += 1
        
        # CRITICAL: Even if timeout, check if we actually connected
        # Sometimes Wavelink 3.x connects but doesn't signal properly
        player = interaction.guild.voice_client
        if player and player.channel:
            log(f"⚠️ Timeout occurred but connection detected! Proceeding...")
            player.home = interaction.channel
            await player.set_volume(VOLUME_ON_JOIN)
            await apply_audio_settings(player)
            return player, None
        
        return None, "❌ **Voice connection failed.**\n\nThis is a known Wavelink 3.x issue. Solutions:\n1. Restart the bot\n2. Try a different voice region\n3. Check Lavalink connection\n4. Use Wavelink 2.x instead"
        ...
        if "timeout" in error.lower():
            return None, "❌ Connection timeout. Try again or restart bot."
        elif "already connected" in error.lower():
            return interaction.guild.voice_client, None
