...
# ============== VOICE CONNECTION - EXACTLY LIKE OLD WORKING CODE ==============
async def connect_voice(interaction: discord.Interaction):
    """
    EXACT pattern from old working files:
    - Simple connect with cls=wavelink.Player
    ...
    try:
        ...
        # EXACTLY like old working code - simple connect, no timeout, no extras
        player = await voice_channel.connect(cls=wavelink.Player)
        
        # Set home for notifications
        ...
        # If timeout error, suggest solutions
        if "timeout" in error.lower():
            return None, "❌ **Voice connection timeout.**\n\nThis usually means:\n1. Discord Voice State Intent is NOT enabled\n2. Bot token doesn't have voice permissions\n3. Hosting provider blocks UDP (try different host)\n\nCheck Discord Developer Portal → Bot → Voice State Intent!"
...
@bot.tree.command(name="1000)}ms | Streams: {active} | Servers: {len(self.guilds)}")
            except:
                pass
            await asyncio.sleep(30)
