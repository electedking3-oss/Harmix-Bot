...
# ============== COMPLETELY REWRITTEN VOICE CONNECTION ==============
async def ensure_voice_connection(interaction: discord.Interaction) -> tuple[wavelink.Player | None, str | None]:
    """
    Completely rewritten voice connection system with proper retry logic.
    Returns: (player, error_message)
    """
    log(f"🔊 Voice request by {interaction.user} in guild {interaction.guild.id}")
    ...
    existing_player = wavelink.Pool.get_node().get_player(guild_id)
    if existing_player and existing_player.connected:
        log(f"✅ Already connected to {existing_player.channel.name}")
        # Move to new channel if different
        if existing_player.channel.id != user_channel.id:
            log(f"🔄 Moving to {user_channel.name}")
            await existing_player.move_to(user_channel)
        return existing_player, None

    # Check for existing discord.py voice client and disconnect it
    if interaction.guild.voice_client:
        log("🧹 Cleaning up old voice client")
        try:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            log(f"🔄 Connection attempt {attempt}/{max_retries} to '{user_channel.name}'")

            # Create player by connecting to voice channel
            ...
            if not player.connected:
                log(f"⚠️ Player reports not connected after attempt {attempt}")
                if attempt < max_retries:
                    await asyncio.sleep(1)
                    continue
                return None, "❌ **Voice connection failed after multiple attempts.**"

            # Set home channel for notifications
            ...
            log(f"✅ Successfully connected to {user_channel.name}")
            return player, None

        except asyncio.TimeoutError:
            log(f"⏱️ Timeout on attempt {attempt}", "ERROR")
            if attempt == max_retries:
                return None, "❌ **Connection timed out.** Check Discord Voice State Intent in Developer Portal!"
            await asyncio.sleep(2)

        except Exception as e:
            error_str = str(e)
            log(f"❌ Connection error (attempt {attempt}): {error_str}", "ERROR")
            ...
            if attempt == max_retries:
                ...
                if "timeout" in error_str.lower():
                    return None, "❌ **Voice connection timeout.**\n\nCheck: Developer Portal → Bot → Privileged Gateway Intents → **VOICE STATE INTENT** must be ON!"
...
async def play(interaction: discord.Interaction, query: str):
    """Fixed /play command with proper voice connection handling"""
    log(f"🎵 Play: '{query[:50]}...'")
    ...
    player, error = await ensure_voice_connection(interaction)
    if error:
    ...
    try:
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            if added > 0:
                await interaction.followup.send(f"📋 **Added {added} tracks to queue**")
        else:
            # Single track
            track = tracks[0]
            ...
            if not player.playing:
                ...
                await player.queue.put_wait(track)
                title, color = "📝 Added to Queue", COLOR_QUEUE
...
@bot.tree.command(name="skip", description="⏭️ Skip current track")
...
@bot.tree.command(name="volume", description=f"🔊 Set volume (0-{MAX_VOLUME})")
@discord.app_commands.describe(volume="Volume level (0-200)")
async def volume(interaction: discord.Interaction, volume: int):
    if not 0 <= volume <= MAX_VOLUME:
        return await interaction.response.send_message(f"❌ Volume must be 0-{MAX_VOLUME}!", ephemeral=True)
...
async def loop(interaction: discord.Interaction, mode: discord.app_commands.Choice[str]):
    ...
    desc = {"off": "Loop disabled", "track": "Repeating current track", "queue": "Repeating entire queue"}
...
@bot.tree.command(name="stop", description="⏹️ Stop and clear queue")
...
log("=" * 50)
log("🎶 HARMIX STARTING - VOICE CONNECTION FIXED")
log("=" * 50)
